...
export default function Sidebar() {
  ...
  const navItems = [
    { to: '/taskflow', label: 'TaskFlow App', icon: TaskFlowIcon, external: 'http://localhost:3000' },
    { to: '/', label: 'ZenNotes App', icon: ZenNotesIcon, active: true },
  ];
  ...
  return (
    <aside className="fixed left-0 top-0 h-screen w-64 bg-bg-sidebar border-r border-border-subtle flex flex-col z-50">
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-3 rounded-lg text-sm font-medium transition-all duration-200 ${
                  isActive
                    ? 'text-accent-emerald bg-accent-emerald/[0.1] shadow-[inset_0_0_20px_rgba(16,185,129,0.05)]'
      ...
      <div className="p-5 border-t border-border-subtle flex items-center gap-3 bg-white/[0.01]">
        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-accent-emerald to-accent-cyan flex items-center justify-center text-white font-bold shadow-md">
